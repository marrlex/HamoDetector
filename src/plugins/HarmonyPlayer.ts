...
export class HarmonyPlayer {
  ...
  public ana: AnalyserNode
  private playingAudioNodes: Map<number, {osc: OscillatorNode; gain: GainNode}>
  private _attackTimeConst: number
  private _releaseTimeConst: number
  private _isMajor: boolean
  private _stdFreq: number
  private _wavetype: OscillatorType
  private _musicalKey: NoteName
  ...
  constructor ({
    ...
    this.ana = this.audioCtx.createAnalyser()
    this.ana.connect(this.audioCtx.destination)
    this.masterGain = this.audioCtx.createGain()
    this.masterGain.gain.value = 0.1
    this.masterGain.connect(this.ana)
